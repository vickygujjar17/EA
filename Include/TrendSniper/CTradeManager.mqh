//+------------------------------------------------------------------+
//|                                                 CTradeManager.mqh |
//|             Module 6 — Unlimited Trend Runner (Step-Up Trail)      |
...
//+------------------------------------------------------------------+
//| CTradeManager class                                               |
//| Step-up trailing stop: Half-Risk → Breakeven → ATR Trail           |
//+------------------------------------------------------------------+
class CTradeManager
  {
private:
   ...
   bool              ContinueTrail(SActiveTrade &trade)
     {
      ...
      // CRITICAL: Never move backward — only tighten
      if(trade.direction == DIR_LONG)
        {
...
public:
   ...
   //+------------------------------------------------------------------+
   //| Main update — manages the trailing stop for active trades          |
   //+------------------------------------------------------------------+
   void              Update(void)
     {
      ...
      if(!PositionSelectByTicket(trade.ticket))
        {
         ...
         if(EnableLogging)
            Print("[TradeManager] Position ", trade.ticket, " no longer exists — clearing state");
      ...
      switch(trade.trailStep)
        {
         ...
         case STEP_BREAKEVEN:
           // Check if we've reached Step 3 (2.7R) → activate trail
