//+------------------------------------------------------------------+
//|                                                 CRiskManager.mqh  |
//|                              Module 2 — Dynamic 0.25% Risk Sizing  |
   ...
   double            CalculateLotSize(double   entryPrice,
                                      double   stopLossPrice)
     {
      ...
      if(tickValuePerLot > 0)
        {
         // All good — use directly
        }
      else
        {
         Print("[RiskManager] WARNING: tickValue is zero — using fallback calculation");
      ...
      if(finalLotSize < m_volumeMin)
        {
         Print("[RiskManager] WARNING: Calculated lot ", DoubleToString(finalLotSize, 4),
               " < Min ", DoubleToString(m_volumeMin, 2), " — using minimum");
      ...
      if(finalLotSize > m_volumeMax)
        {
         Print("[RiskManager] WARNING: Calculated lot ", DoubleToString(finalLotSize, 4),
               " > Max ", DoubleToString(m_volumeMax, 2), " — using maximum. Risk may exceed target.");
      ...
      if(finalLotSize <= 0)
        {
         Print("[RiskManager] FATAL: Lot size is zero — using minimum");
         finalLotSize = m_volumeMin;
        }
      ...
      if(actualRiskPct > SafetyMaxRiskPct)
        {
         Print("[RiskManager] SAFETY CLAMP: Actual risk ", DoubleToString(actualRiskPct, 2),
               "% exceeds max ", SafetyMaxRiskPct, "% — aborting trade (lot=",
