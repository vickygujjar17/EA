//+------------------------------------------------------------------+
//|                                                   CNewsFilter.mqh |
//|                                 Module 1 â€” High-Impact News Filter |
...
private:
   ...
   //| Uses CalendarCountryById â€” guaranteed in all MT5 builds            |
   //+------------------------------------------------------------------+
...
public:
   ...
   //+------------------------------------------------------------------+
   //| Initialize â€” parse symbol currencies and map to country codes     |
   //+------------------------------------------------------------------+
   ...
   //+------------------------------------------------------------------+
   //| Main update â€” called every tick. Checks news calendar.            |
   //+------------------------------------------------------------------+
   void              Update(void)
     {
      ...
      for(int c = 0; c < m_countryCount && !foundBlackout; c++)
        {
         ...
         // We scan by iterating event IDs â€” in production, you'd use
      ...
      for(int eventId = 1; eventId <= eventsToCheck; eventId++)
        {
         ...
         if(event.country_id == 0 || event.country_id == targetCountryId)
           {
            // Check importance â€” we want HIGH impact only
