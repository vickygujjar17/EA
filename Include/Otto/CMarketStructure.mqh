...
//|                    Module 3 â€” Fractal Pivots & Dominant Peak Memory |
...
private:
   ...
   // --- v4.70 single-pivot settings ---
   int               m_leftBars;         // Left bars for the single 8/3 pivot (Wick 1 & Wick 2)
   int               m_rightBars;        // Right bars for the single 8/3 pivot (Wick 1 & Wick 2)
   ...
   bool              IsFractalHigh(int barIndex, int leftBars, int rightBars)
     {
      ...
      // Check left side â€” all bars must have strictly lower highs
      ...
      // Check right side â€” all bars must have strictly lower highs
      ...
      while(count > maxAllowed)
        {
         ...
         if(oldestIdx >= 0)
           ...
           {
            // No removable peak â€” break to avoid infinite loop
            break;
           }
        }
     }
   ...
   void              ApplyResistanceRetention(SFractalPeak &newPeak)
     {
      ...
      // 1. If no active anchor â†’ store as active
      // 2. If new is higher than active â†’ replace
      // 3. If active is stale (> 30 bars) â†’ replace
      // 4. If active was already triggered â†’ replace
      // 5. Otherwise â†’ discard new peak

      int activeIdx = FindActiveResistPeak();

      if(activeIdx < 0)
        {
         // No active resistance anchor â€” store this one
      ...
      // Rule 2: New peak is higher â†’ dominates
      ...
      // Rule 5: Discard new peak â€” active is dominant
   ...
   void              ApplySupportRetention(SFractalPeak &newPeak)
     {
      ...
      // Rule 2 (mirrored): New low is LOWER â†’ dominates
      if(newPeak.price < active.price)
        {
         shouldReplace = true;
      ...
      for(int i = m_resistCount - 1; i >= 0; i--)
        {
         ...
         m_resistPeaks[i].barIndex++; // Increment relative bar index since a new bar has closed
      ...
      for(int i = m_supportCount - 1; i >= 0; i--)
        {
         ...
         m_supportPeaks[i].barIndex++; // Increment relative bar index since a new bar has closed
...
public:
   ...
   //| Initialize                                                        |
   //+------------------------------------------------------------------+
   bool              Initialize(string symbol, int leftBars, int rightBars)
     {
      m_symbol = symbol;
      m_leftBars = leftBars;
      ...
      if(EnableLogging)
        {
         Print("[MarketStructure] Initialized for ", m_symbol,
               " | Pivot: ", m_leftBars, "/", m_rightBars,
               " (v4.70 single Left/Right pivot for Wick 1 & Wick 2)",
   ...
   //+------------------------------------------------------------------+
   //| Main update â€” scans for fractals, applies retention rules          |
   //+------------------------------------------------------------------+
   void              Update(void)
     {
      m_loopCounter++;
      ...
      for(int bar = scanStart; bar <= scanEnd && bar < LookbackBars; bar++)
        {
          ...
          if(IsFractalLow(bar, m_leftBars, m_rightBars))
            {
             bool alreadyStored = false;
   ...
   //+------------------------------------------------------------------+
   //| Gets the active resistance peak (if any) â€” direct pointer to array |
   //+------------------------------------------------------------------+
