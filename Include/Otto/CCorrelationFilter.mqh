//+------------------------------------------------------------------+
//|                                           CCorrelationFilter.mqh  |
//|              Module â€” Weighted Correlation Matrix (-3 to +3)      |
//|                             Otto EA - Institutional Grade   |
//+------------------------------------------------------------------+
#property copyright "Otto EA"
#property version   "1.00"

#ifndef __CORRELATION_FILTER__
#define __CORRELATION_FILTER__

#include "CommonDefines.mqh"

//+------------------------------------------------------------------+
//| CCorrelationFilter class                                          |
...
private:
   ...
   //+------------------------------------------------------------------+
   //| MODULE 1: GetCorrelationScore â€” Weighted -3 to +3 matrix lookup  |
   //| Returns the correlation score between sym1 and sym2               |
   ...
   //+------------------------------------------------------------------+
   //| MODULE 2: Scans all open positions (MagicNumber) for conflicts    |
   //| Using weighted correlation: |score| >= 2 triggers veto            |
   //+------------------------------------------------------------------+
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
        {
         ...
         // |score| >= 2 is strong correlation â€” apply veto
         if(proposedDir == 1)  // Proposed LONG
           {
            if(posDir == 1 && score <= -2) return true;  // Both LONG, strong negative correlation
            if(posDir == -1 && score >= 2) return true;   // Opposite direction, strong positive correlation
           }
         if(proposedDir == -1) // Proposed SHORT
           {
            if(posDir == -1 && score <= -2) return true;  // Both SHORT, strong negative correlation
            if(posDir == 1 && score >= 2) return true;    // Opposite direction, strong positive correlation
           }
        }
      return false;
...
public:
   ...
   //+------------------------------------------------------------------+
                     CCorrelationFilter(void)
     {
      ...
      m_allSymbols[11] = "XAUUSD";
                    ...
                    ~CCorrelationFilter(void) { ArrayFree(m_allSymbols); }
   ...
   //+------------------------------------------------------------------+
   //| MODULE 2: IsTradeVetoed â€” Weighted Portfolio Veto                 |
   //| Veto only if |correlation score| >= 2 (strong correlation)        |
   //+------------------------------------------------------------------+
   ...
   //+------------------------------------------------------------------+
   //| BroadcastBias â€” Write bias to MT5 Global Variable                 |
   //+------------------------------------------------------------------+
   void              BroadcastBias(int bias)
     {
      string varName = "TS_Bias_" + m_symbol;
      if(bias == 0)
   ...
   //+------------------------------------------------------------------+
   //| MODULE 3: GetWeightedBiasSum â€” Matrix-weighted peer bias sum      |
   //| TotalBias = sum(PeerBias * CorrelationScore) for ALL 11 peers     |
   //+------------------------------------------------------------------+
      ...
      for(int i = 0; i < ArraySize(m_allSymbols); i++)
        {
         ...
         string peerSym = m_allSymbols[i];
         string varName = "TS_Bias_" + peerSym;
         if(GlobalVariableCheck(varName))
           {
            double raw = GlobalVariableGet(varName);
            int peerBias = (int)raw;
            int score = GetCorrelationScore(m_symbol, peerSym);
   ...
   //+------------------------------------------------------------------+
   //| MODULE 3: ResolveBidirectionalConflict â€” Weighted Tie-Breaker     |
   //| TotalBias >= 2  â†’ keep Support (Long), delete Resistance          |
   //| TotalBias <= -2 â†’ keep Resistance (Short), delete Support         |
   //| -1 <= TotalBias <= 1 â†’ delete BOTH (market too mixed)             |
   //+------------------------------------------------------------------+
   int               ResolveBidirectionalConflict(void)
     {
      ...
      if(totalBias >= 2) return 1;    // Strongly Long â†’ keep Support
      if(totalBias <= -2) return -1;   // Strongly Short â†’ keep Resistance
      return 0;                        // Mixed â†’ delete both
     }
